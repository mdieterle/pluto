	...
	if (lst_command[i] == COMMAND_PLUS) {
	    ...
	    *val = lst_form[i]->Eval(1.);
		...
		if (current == '$')
			...
			if (file) {
			    if (x) {
				if (format_puffer_pointer<2) fprintf(file,"%#g", *x);
				else {
...
Bool_t PBatch::AddCommand(char * command) {
	...
	if (strlen(command) > 8) {
		...
		if (current == '@') {
			...
			else Error("AddCommand","Readline: puffer %s not initialized",puffer);
	...
	if (!strncmp(command,"eval()",6) || !strncmp(command,"Eval()",6)) {
	    ...
	    char *prodx[4];
	    Int_t prodx_s=4; //max 4 products (see test below)
...
//	GetKey(prod[0],0,1);
	...
	if (is_value_result_type) {
		...
		if (is_operator && (strcmp(prod[0],"t") == 0 || strcmp(prod[0],"x") == 0 
		    ...
		    Warning("AddCommand","[%s] The '%s' is a reserved keyword in TFormula. You might run into problems.",command,
			    prod[0]);
		}
	    }
	}
