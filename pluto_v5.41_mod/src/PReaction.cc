...
void PReaction:: InitLoop() {
    ...
    else if (asciiOUTPUT==2) asciif=fopen(file1,"wb");
...
////////////////////////    End of Prologue  ////////////////////////////
	...
	if ((error_count_failed > max_failed_events-0.5) && (!good_event)) {
	    status=3;
	...
	for (j=start;j<nchan;++j) {
		...
		if (ret!=8) {
		    last_nonempty = i;
		    //cout << "goto repeat, code=" << ret << endl;
...
//		cout << "found stable: " << particle_stack[k]->ID() << endl;
	...
	if (asciiOUTPUT == 1) {                      // write event to ASCII file
        ...
        if (asciiOUTPUT == 2) {                          // write event to binary file
            if (!getVERTEX) {
                marker = 4*sizeof(Double_t)+sizeof(Int_t);
                if (!allPARTICLES) {
                    for (k=0;k<size_tracked;++k) {               // tracked particles only
                        PParticle *pt=stable_particle[k];
                        if (pt->IsActive()==kFALSE) continue;    // skip inactive particles
                        ...
                        e = pt->E();
                        id = pt->ID();
                        fwrite(&marker, sizeof(Int_t), 1, asciif);
                        fwrite(&px, sizeof(Double_t), 1, asciif);
                        fwrite(&py, sizeof(Double_t), 1, asciif);
                        fwrite(&pz, sizeof(Double_t), 1, asciif);
                        fwrite(&e, sizeof(Double_t), 1, asciif);
                        fwrite(&id, sizeof(Int_t), 1, asciif);
                        fwrite(&marker, sizeof(Int_t), 1, asciif);
                    }
                } else {
                    for (k=0;k<size;++k) {               // tracked particles only
                        PParticle *pt=particle_stack[k];
                        if (pt->IsActive()==kFALSE) continue;    // skip inactive particles
                        px = pt->Px();
                        py = pt->Py();
                        pz = pt->Pz();
                ...
                marker = 7*sizeof(Double_t)+2*sizeof(Int_t);
                if (!allPARTICLES) {
                    for (k=0;k<size_tracked;++k) {               // tracked particles only
                        PParticle *pt=stable_particle[k];
                        if (pt->IsActive()==kFALSE) continue;    // skip inactive particles
                        ...
                        vtx_y = pt->Y();
                        vtx_z = pt->Z();
                        p_id = pt->GetParentIndex()+1;

//                        printf("Particle %i:\n", k+1);
//                        printf("    Px:%11.6f Py: %11.6f Pz:  %11.6f\n", px, py, pz);
//                        printf("    Vx:%11.6f Vy: %11.6f Vz:  %11.6f\n", vtx_x, vtx_y, vtx_z);
//                        printf("    Id:%02i   PId: %02i\n", id, p_id);
                        ...
                        fwrite(&e, sizeof(Double_t), 1, asciif);
                        fwrite(&id, sizeof(Int_t), 1, asciif);
                        fwrite(&vtx_x, sizeof(Double_t), 1, asciif);
                        fwrite(&vtx_y, sizeof(Double_t), 1, asciif);
                        fwrite(&vtx_z, sizeof(Double_t), 1, asciif);
                        fwrite(&p_id, sizeof(Int_t), 1, asciif);
                        fwrite(&marker, sizeof(Int_t), 1, asciif);
                    }
                } else {
                    for (k=0;k<size;++k) {               // tracked particles only
                        PParticle *pt=particle_stack[k];//stable_particle[k];
                        if (pt->IsActive()==kFALSE) continue;    // skip inactive particles
                        px = pt->Px();
                        py = pt->Py();
                        pz = pt->Pz();
