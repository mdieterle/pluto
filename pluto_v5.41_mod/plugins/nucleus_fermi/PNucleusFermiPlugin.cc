...
Bool_t PNucleusFermiPlugin::Activate(void) {
    ...
    if (!makeStaticData()->IsParticleValid("2He")) {
        makeStaticData()->AddParticle(400, "2He",1.87224);
        makeStaticData()->AddAlias("2He","He2");
    }   
	...
	new PFermiDistributions("d_fermi@d/fermi","Fermi momentum of d",-1);
    model->SetRange(0,2.);
    pdmutil->Add(model);
    model = new PFermiDistributions("3He_fermi@3He/fermi","Fermi momentum of 3He",-1);
    model->SetRange(0,2.);
    pdmutil->Add(model);				 
...
Bool_t PNucleusFermiPlugin::ExecCommand(const char * command, Double_t value) {
	...
	//D (n spectator)
	//*****************
	makeStaticData()->AddParticle(45001,"g + d",1.875613);
	makeStaticData()->AddAlias("g + d","g+d");
	makeStaticData()->AddDecay(-1, "g + d -> (g + n) + p (quasi-free)",
				   "g + d","g + n,p", 1.0 );
	PFermiMomentumGA *pmodel = 
	    new PFermiMomentumGA("gn_in_d@g + d_to_g + n_p",
	...
	//(p spectator)
	makeStaticData()->AddDecay(-1, "g + d -> (g + p) + n (quasi-free)",
				   "g + d","g + p,n", 1.0 );
	pmodel = 
	    new PFermiMomentumGA("gp_in_d@g + d_to_g + p_n",
				 "Quasi-free particle production <nucleus_fermi>",-1);	
	// Now add all particles
	// Define spectators and final decay products (the granddaughters)
	pmodel->Add("q,parent");                                       
	...
	pdmutil->Add(pmodel);
        ...
        //(2He spectator)
        makeStaticData()->AddDecay(-1, "g + 3He -> (g + n) + 2He (quasi-free)",
				   "g + 3He","g + n,2He", 1.0 );
	pmodel = 
	    new PFermiMomentumGA("gn_in_3He@g + 3He_to_g + n_2He",
				 "Quasi-free particle production <nucleus_fermi>",-1);
	pmodel->Add("q,parent");                                       
	pmodel->Add("g,grandparent,beam");                            
	pmodel->Add("3He,grandparent,target");
	pmodel->Add("2He,daughter,spectator");
	pmodel->Add("q,daughter,composite");
	pmodel->Add("n,granddaughter,participant"); 
	pmodel->Add("g,granddaughter,p2");
	pdmutil->Add(pmodel);
